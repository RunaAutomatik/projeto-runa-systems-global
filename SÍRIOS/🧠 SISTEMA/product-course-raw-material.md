...
## [2026-04-10] — Claude Code Access Modes: The 4-Mode Framework

**What was built/decided:**
Mapped and documented the 4 distinct ways to access Claude, each with different capabilities, access scope, and output. This distinction is non-obvious to most users who default to the App and never discover the operational power of Terminal + Chrome modes.

**Product module this feeds:**
RUNA SYSTEMS Core Modules — specifically the transition from Modules 1/2 (Claude.ai chat/co-work) to Modules 3/4 (Claude Code Terminal + Antigravity). The Chrome Extension is advanced material within Module 3.

**Raw material value:**
Most people use 20% of Claude (App mode only). The 4-mode framework shows the full capability surface. This is a direct unlock — clients who understand this framework immediately see 5-10x more operational value from the tool they're already paying for. Nobody teaches this systematically.

**Key artifacts:**
- `SÍRIOS/📦 Entregáveis/runa-systems-skool/core-01-claude-access-modes.md` — full lesson with comparative table, analogies, use cases, and RUNA workflow integration
- `SÍRIOS/📚 Referências/dexter-ai-reference.md` — Dexter AI tool reference (adjacent streaming AI, strong AGENT$ case study)
- Chrome Extension: `claude --chrome` (Extension ID: fcoeoabgfenejglbffodgkkbkcdhcgfn), requires Claude Code 2.0.73+ and Extension 1.0.36+, Pro/Max/Team/Enterprise plan, Chrome/Edge only (no WSL)

**The 4 modes summary:**
1. **App (claude.ai)** — text only, no file system, no execution, no MCPs
2. **Terminal (claude)** — full file system + bash + MCPs + skills + hooks + AIOX
3. **IDE Extension (Antigravity/VS Code)** — Terminal + open file context + linter errors + cursor position
4. **Chrome Extension (claude --chrome)** — Terminal + 17 browser tools (navigate, screenshot, click, read_page, read_console, read_network, javascript_tool, form_input, gif_creator...)

**Architecture insight (Native Messaging):**
Claude Code CLI → Socket Bridge → Native Host Process (local binary) → Native Messaging API → Chrome Extension (Content Script). This is why the Desktop App conflicts with `claude --chrome` — they compete for the same socket.
